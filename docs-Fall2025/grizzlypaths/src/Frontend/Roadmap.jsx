...
const CourseCard=memo(function CourseCard({code,name,skills}){
  ...
  return(
    <Card
      ...
      weight={1}
      accent="#95a"
      info={skills?.length?`Skills: ${skills.join(" • ")}`:"No listed skills."}
    />
  );
});
...
export default function Roadmap({onBack}){
  ...
  const[majorId,setMajor]=useState(null),
        [activeType,setActiveType]=useState(null);
  ...
  const bucketCounts=useMemo(()=>{
    ...
    const min=rows.length?Math.min(...rows.map(r=>r[1])):1,
          max=rows.length?Math.max(...rows.map(r=>r[1])):1;
  ...
  const hardWeights=useMemo(()=>{
    ...
    return skills.map(s=>{
      ...
      return{skill:s,count:c,weight:toWeightSkill(c,min,max)};
    });
  },[typeSkills]);

  const softSkills=useMemo(()=>{
    const desc=JOBS
      .filter(j=>activeType?mapToType(majorId,String(j.job_title||""))===activeType:true)
      .map(j=>j.job_description||"")
      .join("\n");
    ...
    <div>
      ...
      <div className="wrap">
        <section className="stage" aria-live="polite">
          ...
          <div className="grid-row majors-row">
            {MAJORS.map(m=>(
              <Card
                ...
                onToggle={()=>setMajor(majorId===m.id?null:m.id)}
                accent={m.color}
              />
            ))}
          </div>
          ...
          {majorId&&<>
            <h2 ref={jobTypesRef} style={{marginTop:16}}>
              Job Types in {MAJORS.find(m=>m.id===majorId)?.label}
            </h2>
          ...
          {activeType&&<>
            <h2 ref={skillsRef} style={{marginTop:18}}>
              Skills for “{activeType}”
            </h2>
            ...
            <div className="grid">
              {courses.map(code=>{
                ...
                const display=(raw||[])
                  .filter(s=>typeSkills.allowed.has(
                    KNOWN.find(k=>norm(k)===norm(s))||"__none__"
                  ))
...
/* styles (minified to fewer lines) */
const GLOBAL_CSS=`
